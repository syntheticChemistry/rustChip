...
impl SoftEsn {
    ...
    fn step(&mut self, input: &[f32]) -> Vec<f32> {
        ...
        for i in 0..self.rs {
            for j in 0..self.is {
                pre[i] += self.w_in[i * self.is + j] * input[j];
            }
            for j in 0..self.rs {
...
impl ApproachBEsn {
    ...
    fn step(&mut self, input: &[f32]) -> Vec<f32> {
        ...
        for i in 0..rs {
            for j in 0..self.is {
                hw[i] += self.w_in_sc[i * self.is + j] * input[j];
            }
            for j in 0..rs {
                hw[i] += self.w_res_sc[i * rs + j] * self.state[j];
            }
...
impl NativeEsn {
    fn from_soft(soft: &SoftEsn) -> Self {
        Self {
            ...
            is: soft.is,
            os: soft.os,
            w_in: soft.w_in.clone(),
            w_res: soft.w_res.clone(),
            w_out: soft.w_out.clone(),
            state: vec![0.0; soft.rs],
            leak: soft.leak,
        }
    ...
    fn step(&mut self, input: &[f32]) -> Vec<f32> {
        ...
        (0..self.os)
            .map(|i| {
                self.w_out[i * self.rs..(i + 1) * self.rs]
                    .iter()
                    .zip(self.state.iter())
                    .map(|(w, s)| w * s)
                    .sum()
...
fn eval_accuracy(
    ...
    for k in 0..n_samples + warmup {
        ...
        if k >= warmup {
            if sw_out[0].signum() as i32 == label {
                sw_correct += 1;
            }
            if b_out[0].signum() as i32 == label {
                b_correct += 1;
            }
            if nat_out[0].signum() as i32 == label {
                nat_correct += 1;
            }
        }
    }
...
fn task_accuracy(rng: &mut Xoshiro) -> bool {
    ...
    println!(
        "  Best ε = {best_eps:.4} → Approach B accuracy = {:.1}%",
        best_accuracy * 100.0
    );
...
fn task_linear_region(rng: &mut Xoshiro) -> bool {
    ...
    println!(
        "  {:>8}  {:>16}  {:>16}  {:>10}",
        "ε", "max_pre_act", "relu_linearity_%", "verdict"
    );
    ...
    for &eps in &epsilons {
        ...
        for _ in 0..100 {
            ...
            for i in 0..rs {
                ...
                if pre < 0.0 {
                    total_nonlinearity += (-pre) / (pre.abs() + 1e-6);
                }
                n_probes += 1;
            }
        }
        let avg_nonlinearity = total_nonlinearity / n_probes as f32 * 100.0;
        if avg_nonlinearity > worst_nonlinearity {
            worst_nonlinearity = avg_nonlinearity;
            worst_eps = eps;
        }

        let verdict = if avg_nonlinearity < 5.0 {
            "✅ linear"
        } else {
            "⚠️ nonlinear"
        };
...
fn task_epsilon_sweep(rng: &mut Xoshiro) -> bool {
    ...
    let max_weight = soft
        .w_in
        .iter()
        .chain(soft.w_res.iter())
        .map(|x| x.abs())
        .fold(0.0f32, f32::max);
    println!("  max |weight| = {max_weight:.4}");

    println!(
        "  Auto-ε formula: ε = 0.02 / max_weight = {:.5}",
        0.02 / max_weight
    );
    println!();
    ...
    println!(
        "  {:>8}  {:>18}  {:>22}",
        "ε", "int4_levels_used", "effective_resolution"
    );
    ...
    println!();
    println!(
        "  Recommendation: ε = 0.02 / max_weight = {:.5}",
        0.02 / max_weight
    );
    println!(
        "  At this ε: {:.1}% of int4 range used, linear region ≈ guaranteed",
...
fn main() {
    ...
    if probe.is_available() {
        println!("  {} — Phase 2 path ready.", probe.status_line());
        println!("     Run with --hw to activate hardware dispatch after Phase 2.");
    } else {
        println!(
            "  {} — running Phase 1 software simulation.",
            probe.status_line()
        );
    }
    println!();
