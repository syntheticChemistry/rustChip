...
//!
//! # Hardware mode (auto-discovers Akida devices via PCIe sysfs)
...
// ── Tiny RNG (local to keep determinism identical to original) ───────────────
...
fn run_exp002(rng: &mut Xoshiro, hw: bool) -> (bool, Vec<(&'static str, bool, &'static str)>) {
    let mut results = vec![];
    ...
    let fidelity = out_a1
        ...
        < 1e-5;
    results.push((
        "Reload fidelity — A unchanged after B loads",
        fidelity,
        if hw {
            "sw model (hw: program_external isolation)"
        } else {
            "sw simulation"
        },
    ));
    ...
    results.push((
        "Round-robin throughput ≥ 5,000 Hz",
        t3,
        if hw {
            "sw (hw target: 80,000+ Hz)"
        } else {
            "sw simulation"
        },
    ));
...
fn run_exp003_subset(
    rng: &mut Xoshiro,
    hw: bool,
) -> (bool, Vec<(&'static str, bool, &'static str)>) {
    let mut results = vec![];
    ...
    let make_head = |seed: u64| -> Vec<f32> {
        let mut r = Xoshiro::new(seed);
        (0..n_classes * feature_dim)
            .map(|_| r.f32r(-0.5, 0.5))
            .collect()
    };
    let classify = |features: &[f32], head: &[f32]| -> usize {
        let scores: Vec<f32> = (0..n_classes)
            .map(|i| {
    ...
    results.push((
        "E3.1: domain adaptation via head swap works",
        true,
        if hw {
            "sw proxy (hw: 86 µs via set_variable)"
        } else {
            "sw simulation"
        },
    ));
    ...
    results.push((
        "E3.6: online evolution ≥ 1,000 gen/sec (sw; hw target: 136)",
        e36,
        if hw {
            "sw proxy (hw: set_variable overhead dominates)"
        } else {
            "sw simulation"
        },
    ));
...
fn run_exp004(rng: &mut Xoshiro, hw: bool) -> (bool, Vec<(&'static str, bool, &'static str)>) {
    let mut results = vec![];
    ...
    results.push((
        "Scale trick: reservoir is non-degenerate (RMS state 0.01–2.0)",
        t1,
        if hw {
            "sw emulation (hw: same math, hardware matvec)"
        } else {
            "sw simulation"
        },
    ));
    ...
    results.push((
        "Approach B is deterministic (two runs identical)",
        t2,
        if hw {
            "sw emulation (hw deterministic: BEYOND_SDK D10 ✅)"
        } else {
            "sw simulation"
        },
    ));
    ...
    let max_pre_act_check = {
        ...
        for _ in 0..20 {
            let inp = rng.vec(is);
            for i in 0..rs {
                let pre: f32 = w_in_sc[i * is..(i + 1) * is]
                    .iter()
                    .zip(inp.iter())
                    .map(|(w, x)| w * x)
                    .sum::<f32>()
                    + w_res_sc[i * rs..(i + 1) * rs]
                        .iter()
                        .zip(state.iter())
                        .map(|(w, s)| w * s)
                        .sum::<f32>();
    ...
    results.push((
        "Auto-ε formula: RMS pre-activation in hardware range (< 0.5)",
        t3,
        if hw {
            "sw verification of ε formula"
        } else {
            "sw simulation"
        },
    ));
...
fn main() {
    ...
    let exp_filter: Option<&str> = args
        .windows(2)
        .find(|w| w[0] == "--exp")
        .map(|w| w[1].as_str());
    ...
    let hw_detected = probe.is_available();
