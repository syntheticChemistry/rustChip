...
fn main() -> Result<()> {
    ...
    if use_sw {
        ...
        match DeviceManager::discover() {
            Ok(ref mgr) if !mgr.devices().is_empty() => {
                ...
                println!(
                    "PCIe  : Gen{} x{}",
                    d.capabilities().pcie.generation,
                    d.capabilities().pcie.lanes
                );
            }
            Ok(_) => {
    ...
    suite.run(
        "D1: Channel sweep — throughput sweet spot at 128ch",
        |s| {
            if s.use_sw {
    ...
    suite.run(
        "D3: program_external() — hand-built program executes",
        |s| {
            if s.use_sw {
                ...
                Ok(ValidationResult {
                    passed,
                    message: format!(
                        "SW infer output={:.4} (finite → pass; HW validates FlatBuffer injection)",
                        out[0]
                    ),
                })
            } else {
    ...
    suite.run(
        "D6: Weight mutation — set_variable() swap < 200 µs",
        |s| {
            if s.use_sw {
                let mut sw = make_sw_backend(50, 8, 1);
                ...
                for i in 0..n {
                    if i % 2 == 0 {
                        sw.swap_readout(&w_a).ok();
                    } else {
                        sw.swap_readout(&w_b).ok();
                    }
                }
    ...
    suite.run(
        "D8: BAR layout — BAR0/BAR2/BAR4 at correct offsets",
        |s| {
            if s.use_sw {
                Ok(ValidationResult {
                    passed: true,
                ...
                Ok(ValidationResult {
                    ...
                    message: format!(
                        ...
                        caps.npu_count, caps.memory_mb, caps.pcie.generation, caps.pcie.lanes
                    ),
                })
            }
        },
    ...
    suite.run(
        "D9: Power scaling — Performance > Economy > LowPower",
        |s| {
            if s.use_sw {
                Ok(ValidationResult {
                    passed: true,
    ...
    suite.run(
        "D10: NP mesh — 1000 NPs total (80 DP + 920 NP confirmed)",
        |s| {
            if s.use_sw {
                ...
                Ok(ValidationResult {
                    passed: rs == 50,
                    message: format!(
                        "SW RS={rs} (HW: 1000 NPs = 80 DP-NPs + 920 NPs, from REGISTER_PROBE_LOG)"
                    ),
                })
            } else {
                let mgr = DeviceManager::discover()?;
    ...
    if !use_sw {
        suite.run(
            "Parity: SoftwareBackend output matches hardware (< 5% relative error)",
            |_s| {
                let mut sw = make_sw_backend(50, 8, 1);
                ...
                Ok(ValidationResult {
                    passed,
                    message: format!(
                        "Max relative error SW vs HW: {:.2}% (target <5%)",
                        max_rel * 100.0
                    ),
                })
            },
...
impl ValidationSuite {
    ...
    fn finish(&self) {
        ...
        println!(
            ...
            self.passed, self.failed, self.skipped, self.passed, total
        );
        if self.failed == 0 {
            println!("All checks passed ✓");
        } else {
            println!(
                "VALIDATION FAILED — {} check(s) require attention",
                self.failed
            );
            std::process::exit(1);
        }
...
fn make_sw_backend(rs: usize, is: usize, os: usize) -> SoftwareBackend {
    ...
    let w_out: Vec<f32> = (0..os * rs)
        .map(|i| (i as f32 + 1.0) / (rs as f32 * os as f32))
        .collect();
    let _ = seed; // used for determinism comment
    sw.load_weights(&w_in, &w_res, &w_out)
        .expect("load test weights");
    sw
}
